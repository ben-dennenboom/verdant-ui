...
<x-v-button.base
    {{ $attributes->merge([
        'class' => $outline
            ...
            : 'v-bg-primary-700 dark:v-bg-primary-600 v-border-transparent v-text-white hover:v-bg-primary-800 dark:hover:v-bg-primary-500 focus:v-ring-primary-500' . ($disabled ? ' v-cursor-not-allowed' : '')
    ]) }}
    :href="$href"
    :disabled="$disabled"
    :icon="$icon"
