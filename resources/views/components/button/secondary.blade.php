...
<x-v-button.base
    {{ $attributes->merge([
        'class' => $outline
            ...
            : 'v-bg-secondary-700 dark:v-bg-secondary-600 v-border-transparent v-text-white dark:hover:v-text-black dark:v-text-white hover:v-bg-secondary-800 dark:hover:v-bg-secondary-500 focus:v-ring-secondary-500' . ($disabled ? ' v-cursor-not-allowed' : '')
    ]) }}
    :href="$href"
    :disabled="$disabled"
    :icon="$icon"
