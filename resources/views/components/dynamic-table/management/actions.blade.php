...
>
    ...
    @if(count($actions) > 0)
        <div x-show="open"
             ...
             x-transition
             class="v-absolute v-right-0 v-top-full v-mt-1 v-bg-floating v-rounded-lg v-border v-border-gray-200 dark:v-border-gray-600 v-z-50 v-shadow-lg v-min-w-[180px] v-py-1"
        >
            @foreach($actions as $i => $action)
                @php $disabled = !empty($action['disabled']); @endphp
                @php $icon = $action['icon'] ?? null; @endphp
