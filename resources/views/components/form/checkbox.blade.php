...
<div class="v-mb-4 v-flex v-align-middle v-gap-4 v-items-center">
    ...
    <input type="checkbox" name="{{ $name }}" id="{{ $name }}"
        ...
        {{ $checked ? 'checked' : '' }}
        {{ $required ? 'required' : '' }}
        {{ $attributes->merge(['class' => 'v-rounded focus:v-ring-secondary-500 focus:v-border-secondary-500 v-block v-shadow-sm sm:v-text-sm v-border-secondary-300 v-bg-white v-dark:v-bg-gray-800 v-dark:v-border-gray-600']) }}>
    @error($name)
