@props(['sortable' => false, 'field' => '', 'title' => '', 'filter' => request()->get('filter', []), 'span' => 1])
@php
...
@endphp
<div {{ $attributes->merge(['class' => "v-px-6 v-py-3 v-text-left v-font-medium v-text-sm v-text-gray-500 dark:v-text-gray-400 v-uppercase v-tracking-wider {$spanClass}"]) }}
            ...
            <span class="v-relative v-flex v-items-center">
                @if($sortField === $field)
                    @if($sortDirection === 'asc')
                        <svg class="v-w-3 v-h-3 v-text-gray-700 dark:v-text-gray-300" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    @endif
                @else
                    <svg class="v-w-3 v-h-3 v-text-gray-400 dark:v-text-gray-500 v-opacity-0 group-hover:v-opacity-100" fill="currentColor" viewBox="0 0 20 20"
