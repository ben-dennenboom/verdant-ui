...
<th {{ $attributes->merge(['class' => 'v-px-6 v-py-3 v-text-left v-font-medium v-text-sm v-text-gray-500 v-uppercase v-tracking-wider']) }}>
    @if($sortable)
            ...
            <span class="v-relative v-flex v-items-center">
                @if($sortField === $field)
                    @if($sortDirection === 'asc')
                        <svg class="v-w-3 v-h-3 v-text-gray-700" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                    ...
                    @endif
                @else
                    <svg class="v-w-3 v-h-3 v-text-gray-400 v-opacity-0 group-hover:v-opacity-100" fill="currentColor" viewBox="0 0 20 20"
